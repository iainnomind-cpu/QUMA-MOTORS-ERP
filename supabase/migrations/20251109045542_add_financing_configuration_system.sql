/*
  ...
  1. Nuevas Tablas
    - `financing_rules`
      - Reglas fijas de financiamiento (Corto Plazo Interno, Caja Colón, etc.)
      - Define tasas de interés, plazos y enganches por tipo de financiamiento
      - Permite configurar requisitos de precio mínimo
    
    - `financing_campaigns`
      - Campañas variables de Yamaha/Banco por modelo y periodo
      - Control de vigencia por fechas (ej. Octubre-Diciembre)
      - Aplicabilidad específica por modelo y rango de precio
      - Sistema de prioridad para múltiples campañas activas
    
    - `financing_calculations_log`
      - Registro de todos los cálculos realizados
      - Trazabilidad para Chatbot, Simulador y Agentes
      - Análisis de preferencias de financiamiento
  ...
  3. Important Notes
    - Las reglas fijas codifican los planes estándar de la concesionaria
    - Las campañas variables permiten promociones temporales por modelo
    - El sistema prioriza campañas activas sobre reglas generales
    - Se registra el origen de cada cálculo (simulator, chatbot, agent)
*/
...
-- Policies para acceso público (demo)
...
-- Insertar reglas de financiamiento FIJAS por defecto
...
-- Insertar campaña VARIABLE ejemplo de Yamaha Especial
