/*
  ...
  ## Descripción
  Sistema completo para gestión de refacciones y accesorios con control de stock,
  categorización y seguimiento de ventas directas.
  ...
  - `category` (text: 'refaccion' | 'accesorio')
  - `subcategory` (text, subcategoría específica)
  - `description` (text, descripción detallada)
  - `compatible_models` (text[], modelos compatibles)
  - `brand` (text, marca del producto)
  - `price_retail` (numeric, precio de venta)
  - `cost_price` (numeric, costo de adquisición)
  ...
  - `id` (uuid, primary key)
  - `sale_date` (timestamptz, fecha de venta)
  - `customer_name` (text, nombre del cliente)
  - `customer_phone` (text, teléfono)
  - `customer_type` (text: 'walk-in' | 'cliente' | 'lead')
  - `related_customer_id` (uuid, ID si es cliente/lead registrado)
  ...
  - `payment_method` (text)
  - `notes` (text)
  - `sold_by` (uuid, agente que vendió)
  - `created_at` (timestamptz)

  ### `parts_inventory_movements`
  Historial de movimientos de inventario:
  - `id` (uuid, primary key)
  - `part_id` (uuid, referencia a parts_accessories_inventory)
  - `movement_type` (text: 'entrada' | 'salida' | 'ajuste' | 'venta')
  - `quantity` (integer, cantidad del movimiento)
  - `previous_stock` (integer)
  - `new_stock` (integer)
  - `reason` (text, motivo del movimiento)
  - `reference_id` (uuid, ID de venta u orden relacionada)
  - `performed_by` (uuid, usuario que realizó el movimiento)
  - `created_at` (timestamptz)
  ...
  ## Notas Importantes
  - Sistema diseñado para ventas directas y walk-in customers
  - Control automático de stock al registrar ventas
...
-- Políticas RLS para parts_accessories_inventory
CREATE POLICY "Usuarios autenticados pueden ver inventario"
  ON parts_accessories_inventory
  FOR SELECT
  TO authenticated
  USING (true);
...
-- Políticas RLS para parts_sales
...
-- Políticas RLS para parts_inventory_movements
CREATE POLICY "Usuarios autenticados pueden ver movimientos"
  ON parts_inventory_movements
  FOR SELECT
  TO authenticated
  USING (true);
